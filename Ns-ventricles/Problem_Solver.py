...
def problemsolver(filename, iteration = 0, conv = False, errors = False):
	...
	while t < T:
		...
		# Dirichlet Boundary Conditions vector construction
		        ...
		        a, L = VariationalFormulationMixUnsteadySTOKES(param, u, v,p,q, dt, n, u_old,p_old, K, t, mesh, ds_vent, ds_skull)
...
#def tensor_jump_b(u, n):
  ...
  #sigmav_b=gammav*l*l*mu/h
        ...
        if (mesh.ufl_cell()==triangle):
        	...
        	f = Expression((fx,fy,fz), degree = int(param['Spatial Discretization']['Polynomial Degree velocity']), t=time)
